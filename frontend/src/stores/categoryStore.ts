...
interface CategoryState {
  ...
  findOrCreateCategory: (name: string, color?: string, icon?: string) => Promise<Category>
  clearError: () => void
}
...
export const useCategoryStore = create<CategoryState>()(
  persist(
    (set, get) => ({
      ...
      findOrCreateCategory: async (name: string, color = '#3B82F6', icon = '📁') => {
        ...
        return await createCategory({ name, color, icon })
      },
